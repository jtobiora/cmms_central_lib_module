package ng.upperlink.nibss.cmms.util.emandate.soap.builder;

import com.google.common.collect.ImmutableSet;
import ng.upperlink.nibss.cmms.util.emandate.soap.interfaces.ISoapPayload;
import ng.upperlink.nibss.cmms.util.emandate.soap.util.CaseConverter;
import ng.upperlink.nibss.cmms.util.emandate.soap.util.CollectionUtils;
...
public class SoapBuilder
{
    ...
    private void validateNamespace(Set<String> names) throws Exception{
        ...
        List<String> nameDiff = CollectionUtils.difference(CollectionUtils.union(namespaces,this.namespaces.keySet()),names);
...
xmlContent;
    ...
    public String getXmlSafe() throws Exception {

        try {
            return getXml();
        } catch (Exception e) {
            e.printStackTrace();
            throw new Exception("Soap Compiler error");
        }
    }
    ...
    public SoapPayload getAsPayload() throws Exception{
        return new SoapPayload(this.getXmlAsCDATA(), new LinkedHashMap<>(), false);
    }
    ...
    private String recursiveLookup(SoapPayloadCollection payload,String tag) throws Exception{
        ...
        if(!this.xml_mode) this.validateNamespace(ImmutableSet.of(payload.getNamespace()));
